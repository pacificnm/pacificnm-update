...
use Pacificnm\Update\Service\ServiceInterface as UpdateServiceInterface;
use Pacificnm\Update\Entity\Entity;
use Pacificnm\Install\Service\ServiceInterface as InstallServiceInterface;
...
class ConsoleController extends AbstractActionController
{
    ...
    public function indexAction()
    {
        ...
        foreach ($this->config['module'] as $module) {
            ...
            $moduleSpec = strtolower($filter->filter($module['name']));
            
            // get module file
            $moduleConfigFile = $moduleConfigFile = getcwd() . '/vendor/pacificnm/pacificnm-' . $moduleSpec . '/config/pacificnm.'.$moduleSpec.'.global.php';
                ...
                if ($moduleConfig->module->{$module['name']}->install) {
                    ...
                    if ($moduleConfig->module->{$module['name']}->install->require) {
                        ...
                        foreach ($moduleConfig->module->{$module['name']}->install->require as $requireModule) {
                            ...
                            $requireModuleSpec = strtolower($filter->filter($requireModule));
                            
                            $tempModuleConfigFile = getcwd() . '/vendor/pacificnm/pacificnm-' . $requireModuleSpec . '/config/pacificnm.'.$requireModuleSpec.'.global.php';
                            
                            $tempModuleConfig = new Config(include $tempModuleConfigFile);
                            
                            $requireSqlFile = getcwd() . '/vendor/pacificnm/pacificnm-' . $requireModuleSpec . '/' . $tempModuleConfig->module->{$requireModule}->install->sql;
                    ...
                    $sqlFile = getcwd() . '/vendor/pacificnm/pacificnm-' . strtolower($filter->filter($module['name'])) . '/' . $moduleConfig->module->{$module['name']}->install->sql;
